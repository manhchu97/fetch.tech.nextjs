...
const BlogItem = ({ post }: BlogItemProps): React.ReactElement => {
    const { title, slug, imageCover, description, tags, createdTimestamp, user } = post
    ...
    return <article className={styles["blog-item"]}>
        <div className='blog-item-thumbnail'>
            <Link href='/'>
                <Image alt={slug} src={`http://139.59.117.75:3008/${imageCover}`} width={880} height={400} />
            </Link>
        </div>
        ...
        <div className='blog-item-info'>
            ...
            <div className='blog-item-meta'>
                <div className='blog-meta-tags'>
                    Tags:
                    {formatTags.map(tag => <span key={tag}>{tag}</span>)}
                </div>
                ...
                <div className='blog-meta-author'>
                    <Image alt={name} src={`http://139.59.117.75:3008/${linkAvatar}`} width={25} height={25} />
